...
# Unit Test Script for TaskAttachments Service (matching CI behavior)
# Usage: ./test-unit.sh

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
TASK_ATTACHMENTS_DIR="$(dirname "$SCRIPT_DIR")"
BACKEND_DIR="$(dirname "$TASK_ATTACHMENTS_DIR")"

echo "==> TaskAttachments Unit Tests"

# Change to TaskAttachments directory
cd "$TASK_ATTACHMENTS_DIR"
...
if pytest -q -m unit --maxfail=1 --disable-warnings --junitxml=pytest-report.xml; then
