...
export default function ReportGeneration() {
  ...
  // PDF Export Functions - Now receive data as parameter
  const generateTaskCompletionPDF = async (report: TaskCompletionReport) => {
    ...
    doc.text(`Generated by: ${report.metadata.generated_by || 'System'}`, pageWidth / 2, yPos + 5, { align: 'center' });
    yPos += 20;

    const { summary } = report;
    const stats = [
      ['Total Tasks', summary.total_tasks.toString()],
      ['Completed Tasks', `${summary.completed_tasks} (${summary.completion_rate.toFixed(1)}%)`],
      ['In Progress Tasks', `${summary.in_progress_tasks} (${((summary.in_progress_tasks / summary.total_tasks) * 100).toFixed(1)}%)`],
      ['Blocked Tasks', `${summary.blocked_tasks} (${((summary.blocked_tasks / summary.total_tasks) * 100).toFixed(1)}%)`],
      ['To Do Tasks', `${summary.to_do_tasks} (${((summary.to_do_tasks / summary.total_tasks) * 100).toFixed(1)}%)`],
      ['Active Projects', summary.unique_projects.toString()],
      ['Departments', summary.unique_departments.toString()],
    ];
    ...
    const chartConfig: ChartConfiguration = {
      ...
      data: {
        ...
        datasets: [{
          data: [summary.completed_tasks, summary.in_progress_tasks, summary.to_do_tasks, summary.blocked_tasks],
      ...
      options: {
        ...
        plugins: {
          ...
          title: { display: true, text: 'Task Status Distribution', font: { size: 20, weight: 'bold' }, padding: { bottom: 20 } },
          datalabels: {
            color: '#ffffff',
    ...
    const taskData = report.data.tasks.map((task: TaskDetail) => [
    ...
    const chartConfig: ChartConfiguration = {
      ...
      data: {
        labels: sortedProjects.map((p) => {
          ...
          const name = p.project_name.length > 25 ? p.project_name.substring(0, 25) + '...' : p.project_name;
          return name;
        }),
        datasets: [
          {
